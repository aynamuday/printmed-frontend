...
const PaymentTable = ({ forDashboard, payments, onTogglePaymentStatus }) => {
      ...
      <tbody>
        { payments && payments.length > 0 ? (
          payments.map((payment, index) => (
            <tr key={index}>
              <td className="border p-2 border-[#828282] text-center">{`${!forDashboard ? payment.date + '\u00A0' : ""} ${payment.time}`}</td>
              ...
              <td className="border p-2 border-[#828282] text-center">
                <button
                  ...
                  className={`px-4 py-2 rounded ${payment.is_paid ? 'bg-green-500 text-white' : 'bg-red-500 text-white'}`}
                >
          ...
          <tr>
            <td colSpan="8" className="border p-2 border-[#828282] text-center">No payments found</td>
          </tr>
        )}
      </tbody>
    </table>
